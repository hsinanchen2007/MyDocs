...
struct Goods								//物品结构体类型 
{
	...
	bool operator<(const Goods&b) const		//用于按v/w递减排序 
	{
...
int n=3;							//3种物品
int W=4;							//背包容量为4
vector<Goods> g={Goods(0,2,4),Goods(1,2,3),Goods(2,1,3)};
...
void bound(QNode& e)		//计算第i层结点的上界函数值
{	int rw=W-e.cw;								//背包的剩余容量
	double b=e.cv;								//表示物品价值的上界值
	int j=e.i;
	while (j<n && g[j].w<=rw)
	{	rw-=g[j].w;								//选择物品j
		b+=g[j].v;								//累计价值
...
{	QNode e,e1,e2;					//定义3个结点
	...
	while (!qu.empty())					//队不空循环
	{
		...
		{	e1.cw=e.cw+g[e.i].w;			//选择物品e.i
			...
			printf("   左结点[%d,%d],e1.i=%d,  ",e1.cw,e1.cv,e1.i); 
			EnQueue(e1,qu);
		} 
		else
			printf("   左结点[%d,%d],e1.i=%d ->cut\n",e1.cw,e1.cv,e1.i); 
		e2.cw=e.cw;					//不选择物品e.i
		e2.cv=e.cv;
		e2.x=e.x;
		e2.x[e.i]=0;					//不选择物品i
		...
		bound(e2);
		if (e2.ub>bestv)					//右孩子(不选择物品e.i剪支
		{
			printf("   右结点[%d,%d,%g],e2.i=%d,  ",e2.cw,e2.cv,e2.ub,e2.i); 
			EnQueue(e2,qu);
		} 
		else
			printf("   右结点[%d,%d,%g],e2.i=%d ->cut\n",e2.cw,e2.cv,e2.ub,e2.i);
	}
}
void dispg()					//输出g 
{
