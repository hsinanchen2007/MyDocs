...
template <class T>
Ring<T>& Ring<T>::operator=(const Ring& rhs)
{
  if (size() >= rhs.size())
  {
    while (size() != rhs.size())
...
template <class T>
typename Ring<T>::iterator Ring<T>::insert(const_iterator insertionPoint,
  const T& source)
{
...
template <class T>
typename Ring<T>::iterator Ring<T>::erase(const_iterator erasurePoint)
{
